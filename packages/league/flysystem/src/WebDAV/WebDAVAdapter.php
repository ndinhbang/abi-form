...
class WebDAVAdapter implements FilesystemAdapter
{
     ...
     * @var string
     */
    private $visibilityHandling = self::ON_VISIBILITY_THROW_ERROR;
    /**
     * @var bool
     */
    private $manualCopy = false;
    /**
     * @var bool
     */
    private $manualMove = false;
    public function __construct(Client $client, string $prefix = '', string $visibilityHandling = self::ON_VISIBILITY_THROW_ERROR, bool $manualCopy = false, bool $manualMove = false)
    {
        $this->client = $client;
    ...
    public function listContents(string $path, bool $deep): iterable
    {
        ...
        foreach ($response as $path => $object) {
            ...
            if ($this->propsIsDirectory($object)) {
                ...
                yield new FileAttributes($path, $object['file_size'] ?? null, null, $object['last_modified'] ?? null, $object['mime_type'] ?? null);
            }
        }
    }
